...
class SuperGodSystem:
    ...
    def initialize(self):
        ...
        try:
            ...
            self.market_analyzer = market_analyzer.SuperMarketAnalyzer()
            ...
            self.risk_manager = risk_manager.SuperRiskManager()
            self._log_progress("风险管理系统", 100)
            
            # 初始化仓位管理器
            logger.info(f"{Fore.YELLOW}初始化仓位管理器...{Style.RESET_ALL}")
            self.position_manager = position_manager.SuperPositionManager()
            self._log_progress("仓位管理器", 100)
            
            # 初始化决策引擎
            logger.info(f"{Fore.YELLOW}初始化超神决策引擎...{Style.RESET_ALL}")
            self.decision_engine = decision_engine.SuperGodDecisionEngine()
